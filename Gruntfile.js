module.exports = function(grunt) {
    ...
    var config = {
        ...
        path: {
            ...
            chrome_pem: userhome() + "/.d2ne/chrome.pem", // Generated by Chrome the first time
            openssl: "openssl",
            safari_cert_dir: userhome() + "/.d2ne/safari", // http://developer.streak.com/2013/01/how-to-build-safari-extension-using.html
            xar: userhome() + "/bin/xar", // https://github.com/downloads/mackyle/xar/xar-1.6.1.tar.gz
    ...
    config.compiled_script.outputFile = path.join(config.outputDir, placeholders.compiled_script);

    config.userscript.outputFile = path.join(config.outputDir, "userscript.user.js");
    config.chrome_crx.outputFile = path.join(config.outputDir, "chrome.crx");
    config.chrome_zip.outputFile = path.join(config.outputDir, "chrome.zip");
    config.firefox.outputFile = path.join(config.outputDir, "firefox.xpi");
    config.opera.outputFile = path.join(config.outputDir, "opera.nex");
    config.safari.outputFile = path.join(config.outputDir, "safari.safariextz");

    config.userscript.workingDir = path.join(config.outputDir, "userscript");
    config.chrome_crx.workingDir = path.join(config.outputDir, "chrome");
    config.chrome_zip.workingDir = path.join(config.outputDir, "chrome");
    config.firefox.workingDir = path.join(config.outputDir, "firefox");
    config.opera.workingDir = path.join(config.outputDir, "opera");
    config.safari.workingDir = path.join(config.outputDir, "safari.safariextension");
    ...
    grunt.config.init({
        ...
        shell: {
            ...
            pack_chrome_zip: {
                command: function() {
                    var cmd = 'echo ' + grunt.file.expand(config.chrome_zip.workingDir + '**/*').join(' ') + " | tr ' ' '\n' | " + config.path.zip + " -j " + config.chrome_zip.outputFile + " -@";
                    return cmd;
                }
            },
            pack_firefox: {
            ...
            pack_opera: {
                command: function() {
                    var cmd =
                        ...
                        "mv " + config.outputDir + "/opera.crx " + config.opera.outputFile;
                    return cmd;
                }
            },
            pack_safari: {
                command: function() {
                    var cmd =
                        "digest_file='" + config.safari.workingDir + "/digest.dat';" +
                        "sig_file='" + config.safari.workingDir + "/sig.dat';" +
                        "cd '" + config.safari.workingDir + "/..' && " + config.path.xar + " -czf " + config.safari.outputFile + " --distribution \"$(basename '" + config.safari.workingDir + "')\";" +
                        config.path.xar + " --sign -f '" + config.safari.outputFile + "' --digestinfo-to-sign \"$digest_file\" --sig-size \"$(cat '" + config.path.safari_cert_dir + "/size')\" --cert-loc '" + config.path.safari_cert_dir + "/cert.der' --cert-loc '" + config.path.safari_cert_dir + "/cert01' --cert-loc '" + config.path.safari_cert_dir + "/cert02';" +

                        config.path.openssl + " rsautl -sign -inkey '" + config.path.safari_cert_dir + "/key.pem' -in \"$digest_file\" -out \"$sig_file\" > /tmp/log 2> /tmp/log2;" +
