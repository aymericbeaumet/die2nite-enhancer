...
PLACEHOLDERS=(
  ...
  '__CROSS_ORIGIN_XHR_PERMISSION_JSON_ARRAY__=["http://__CROSS_ORIGIN_XHR_PERMISSION_1__/","http://__CROSS_ORIGIN_XHR_PERMISSION_2__/","http://__CROSS_ORIGIN_XHR_PERMISSION_3__/","http://__CROSS_ORIGIN_XHR_PERMISSION_4__/"]'
  '__CROSS_ORIGIN_XHR_PERMISSION_1__=bbh.fred26.fr'
  '__CROSS_ORIGIN_XHR_PERMISSION_2__=www.oeev-hordes.com'
  '__CROSS_ORIGIN_XHR_PERMISSION_3__=d2n.duskdawn.net'
  '__CROSS_ORIGIN_XHR_PERMISSION_4__=die2nite.gamerz.org.uk'
...
USERSCRIPT="$OUTPUT_DIR/userscript.user.js"
...
function compile_userscript()
{
  userscript_source_dir="$ABSPATH/userscript"
  userscript_build_dir="$OUTPUT_DIR/userscript"
  userscript_file='userscript.user.js'
  userscript_ext="$OUTPUT_DIR/$userscript_file"

  # Abort if script exist
  if [ -r "$userscript_ext" ] ; then return 0 ; fi

  # Gather sources
  cp -r "$userscript_source_dir" "$userscript_build_dir"
  find "$userscript_build_dir" -mindepth 1 -regex '/.*/[0-9]_.*\.js' -exec cat {} \; >> "$userscript_build_dir/$userscript_file"
...
function compile_chrome_ext()
{
  ...
  # Abort if script exist
  if [ -r "$chrome_ext" ] && [ -r "$chrome_zip" ] ; then return 0 ; fi
...
function compile_firefox_ext()
{
  ...
  # Abort if script exist
  if [ -r "$firefox_ext" ] ; then return 0 ; fi
...
function compile_safari_ext()
{
  ...
  # Abort if script exist
  if [ -r "$safari_ext" ] ; then return 0 ; fi
...
function compile_opera_ext()
{
  ...
  # Abort if script exist
  if [ -r "$opera_ext" ] ; then return 0 ; fi
...
while [ -n "$1" ] ; do
  [[ "$1" =~ ^--all|--userscript$ ]]                    && compile_userscript
  [[ "$1" =~ ^--all|--chrome$ ]]  && compile_userscript && compile_chrome_ext
  [[ "$1" =~ ^--all|--firefox$ ]] && compile_userscript && compile_firefox_ext
  [[ "$1" =~ ^--all|--safari$ ]]  && compile_userscript && compile_safari_ext
  [[ "$1" =~ ^--all|--opera$ ]]   && compile_userscript && compile_opera_ext
