Module.register(function() {
    ...
    */
    ...
    function loadCustomChances()
    {
        ...
        else customFinalStatut = statut2;
        
        $('#editor').next().find('.searchResult').show();
        $('#finalStatut').text(customFinalStatut);
        if(storedJob == 1) $('#correct').text(opti);
        else $('#correct').text(customCorrect);
    }
    ...
    function updateSearch()
    {
        ...
        if(storedTomb == 1) $('#storedTomb input').prop( "checked", true );

        if(storedNight == 1) $('#storedNight input').prop( "checked", true );

        if(storedFurtif == 1) $('#storedFurtif input').prop( "checked", true );
        if(storedJob == 2) $('#storedFurtif').show();

        if(storedCP == 1) $('#storedCP input').prop( "checked", true );

        if(storedPhare == 1) $('#storedPhare input').prop( "checked", true );

        if(storedDevastated == 1) $('#storedDevastated input').prop( "checked", true );
    ...
    function loadChances(){
        ...
        var zomb = +$('div.factions div.zombies').text().split(" ")[1];
